...
model DistrictHeatingSystem_noEleBoiler_Dispatch "System model"
  ...
  Components.HeatPump heatPump(
    ...
    use_T_set_in=false,
    use_T_source_in=false)           annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=270,
        origin={-100,0})));
  ...
  Sensors.ValueCross valueCross1
    annotation (Placement(transformation(extent={{-94,20},{-66,40}})));
...
equation
  ...
  connect(heatPump.port_b, multiSensor1.port_a)
    annotation (Line(points={{-100,10},{-100,26}}, color={0,127,255}));
  ...
  connect(jun4.port_1, multiSensor1.port_b) annotation (Line(points={{-54,50},{
          -100,50},{-100,34}}, color={0,127,255}));
...
end DistrictHeatingSystem_noEleBoiler_Dispatch;
