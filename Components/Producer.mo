...
model Producer
  "DH producer model with internal feed water pump. Controls outlet temperature and heat flow rate (positive) added to fluid."
...
equation
  ...
  if use_T_set_in then
    connect(T_set_in, heater.TSet) annotation (Line(points={{-60,70},{12,70},{
            12,-8},{26,-8}}, color={0,0,127}));
  else
    connect(T_set.y, heater.TSet) annotation (Line(points={{-39,40},{12,40},{12,
  ...
  connect(pid.u_m, heater.Q_flow) annotation (Line(points={{-40,-42},{-40,-60},
  ...
  annotation (
    Icon(graphics={
        ...
        Ellipse(
          ...
          fillColor={238,46,47},
          fillPattern=FillPattern.Solid),
        Line(points={{-42,42},{42,-42}}, color={0,0,0}),
        Line(points={{-42,-42},{42,42}}, color={0,0,0}),
        Text(
...
<ul>
<li>SIMC controller tuning rule with one parameter <code>tc</code>.</li>
